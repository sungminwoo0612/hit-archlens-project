...
#### 학습 곡선
![학습 곡선](classification/runs/classify/yolo-cls16/results.png)
*학습 과정에서의 Loss와 Accuracy 변화. Top-5 Accuracy가 75%에 도달하여 상위 5개 예측 중 정답이 포함될 확률이 높음을 보여줍니다.*

#### Confusion Matrix
![Confusion Matrix](classification/runs/classify/yolo-cls16/confusion_matrix_normalized.png)
*정규화된 혼동 행렬. 대각선이 진할수록 해당 클래스의 분류 정확도가 높음을 의미합니다. 64개 클래스에 대한 분류 성능을 시각적으로 확인할 수 있습니다.*

#### 검증 결과 예시
![검증 배치 예측](classification/runs/classify/yolo-cls16/val_batch0_pred.jpg)
...
#### 학습 곡선
![학습 곡선](detection/runs/aws_icon_detector_trial_14/results.png)
...
#### Precision-Recall 곡선
![PR 곡선](detection/runs/aws_icon_detector_trial_14/BoxPR_curve.png)
*Precision-Recall 곡선. 곡선 아래 면적(AUC)이 클수록 모델의 성능이 우수함을 나타냅니다. 119개 클래스에 대한 평균 PR 곡선을 보여줍니다.*

#### F1 Score 곡선
![F1 곡선](detection/runs/aws_icon_detector_trial_14/BoxF1_curve.png)
...
#### Confusion Matrix
![Confusion Matrix](detection/runs/aws_icon_detector_trial_14/confusion_matrix_normalized.png)
*119개 클래스에 대한 정규화된 혼동 행렬. 클래스 간 혼동 패턴을 시각적으로 확인할 수 있습니다. 유사한 서비스들(예: EC2, ECS, EKS) 간의 혼동이 발생할 수 있음을 보여줍니다.*

#### 검증 결과 예시
![검증 배치 예측](detection/runs/aws_icon_detector_trial_14/val_batch0_pred.jpg)
*검증 데이터셋에 대한 바운딩 박스 예측 결과. 각 아이콘의 위치와 클래스가 정확하게 탐지되었는지 확인할 수 있습니다. 바운딩 박스와 클래스 레이블이 표시되어 있습니다.*

#### 학습 배치 예시
![학습 배치](detection/runs/aws_icon_detector_trial_14/train_batch0.jpg)
